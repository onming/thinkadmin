...
define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            //设置游戏下拉
            $.ajax({
                url: 'ajax/getGameMenu',
                type: 'post',
                dataType: 'json',
                success: function (ret) {
                    var id = $('.game-menu').data("id");
                    $.each(ret.list, function (i, j) {
                        var item = '<li><a href="javascript:;" data-type="' + j.id + '">' + j.name + '</a></li>';
                        $(item).appendTo($(".game-menu-list"));
                        if(id == j.id){
                            $('.game-menu').html(j.name);
                        }
                    });
                }
            });
            ...
            //变更游戏
            $(document).on('click', "ul.game-menu-list li a", function () {
                $.ajax({
                    url: 'ajax/changeGame',
                    dataType: 'json',
