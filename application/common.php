...
if (!function_exists('is_online_env')) {
...
if (!function_exists('curl_post_post')) {
...
if(!function_exists('array_under_reset')) {
     ...
     * 以特定键值的数组
     * @param $array 原数组
     * @param $key 特定键（必须原数组的键名）
     * @param int $type 生成数组维数
     * @return array 返回新数组
     */
...
if(!function_exists('format_echarts_data')) {
    /**
     * 转换成图表使用的数据
     *
     * @param $data
     * @return mixed
     */
    function format_echarts_data($data){
        $result = [];
        if (is_array($data) && isset($data[0])) {
            $data_key = array_keys($data[0]);
            foreach ($data as $key => $value) {
                for ($i = 0; $i < count($data_key); $i++) {
                    $result[$data_key[$i]][] = $value[$data_key[$i]];
                }
            }
        }
        return $result;
