...
class Auth extends \fast\Auth
{
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $admin = Session::get('admin');
        // 必须将结果集转换为数组
        $ruleList = collection(\app\admin\model\AuthRule::where('status', 'normal')->where('ismenu', 1)->order(['weigh' => 'desc', 'id'=>'asc'])->cache("__menu__")->select())->toArray();
        foreach ($ruleList as $k => &$v) {
            ...
            if (!empty($v['condition'])) {
                ...
                switch ($condition[0]){
                    case "gameid":
                        if(empty($admin['game_info']["game_id"]) || $admin['game_info']["game_id"] != $condition[1]){
                            unset($ruleList[$k]);
                            continue;
                        }
                        break;
